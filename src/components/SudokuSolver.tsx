...
const SudokuSolver: React.FC = () => {
  ...
  const handleEvaluate = () => {
    setMessage(
      isValidSudoku(grid) ? "Valid Sudoku state." : "Invalid Sudoku state!"
    );
  };
        ...
        if (!copy[row][col]) {
          for (let num = 1; num <= 9; num++) {
            ...
            if (isValidSudoku(copyToSolve) && solveSudoku(copyToSolve)) {
              setGrid(copy);
              explain = `Filled cell [${row + 1}, ${
                col + 1
              }] with ${num}. This is the smallest valid number for this cell.`;
          ...
          if (!found) {
            explain = `No valid number can be placed at cell [${row + 1}, ${
              col + 1
            }].`;
            setExplanation(explain);
          }
  ...
  return (
    ...
    >
      ...
      <Grid
        container
        spacing={0.5}
        sx={{ width: "max-content", mb: 2, flexDirection: "column" }}
      >
        {grid.map((row, i) => (
            ...
            {row.map((cell, j) => (
              ...
              >
                <TextField
                  value={cell}
                  onChange={(e) =>
                    handleChange(i, j, e.target.value.replace(/[^1-9]/g, ""))
                  }
                  inputProps={{
                    maxLength: 1,
      ...
      <Typography color={message.includes("Invalid") ? "error" : "primary"}>
        {message}
      </Typography>
    </Box>
  );
